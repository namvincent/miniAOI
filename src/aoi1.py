...
def correct_color(image):
    try:
        ...
        hsv = cv.cvtColor(cv.cvtColor(
            image, cv.COLOR_GRAY2BGR), cv.COLOR_BGR2HSV)
...
def process_image(image_path, image=None, alpha=1.5, beta=-50.0, rotate=0):
    ...
    if thresh.shape[0] > thresh.shape[1] and thresh.shape[1] < 300:
        ...
        resized = cv.resize(thresh, (int(adjusted.shape[1] * zoom), int(thresh.shape[0] * zoom)),
...
def compare_color_and_save_mask(image, source, roi, threshold=90):
    ...
    try:
        ...
        template_hsv = cv.cvtColor(cv.cvtColor(
            source, cv.COLOR_GRAY2BGR), cv.COLOR_BGR2HSV)

    try:
        image_hsv = cv.cvtColor(image, cv.COLOR_BGR2HSV)
    except:
        image_hsv = cv.cvtColor(cv.cvtColor(
            image, cv.COLOR_GRAY2BGR), cv.COLOR_BGR2HSV)
...
def check_position(image, template, area, original_image):
    ...
    for meth in methods:
        ...
        for pt in zip(*loc[::-1]):
            ...
            # similar_area = cv.cvtColor(similar_area, cv.COLOR_GRAY2BGR)
...
def calculate_async():
    ...
    for checking_type, item in checking_areas:
        ...
        if checking_type == "c":
            ...
            cv.rectangle(final_result_image, top_left,
    ...
    cv.imwrite(f"Results/result.jpg", final_result_image)
...
def aoi():
    ...
    for area in filter(lambda x: x[0] == 's', checking_areas):
        ...
        while thresh < 180:
            ...
            _, img_adjust = cv.threshold(
                gray, thresh, 255, cv.THRESH_BINARY_INV)
